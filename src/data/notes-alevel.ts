...
// OCR A LEVEL (H446)
// ═══════════════════════════════════════════════════════════════════
const OCR_ALEVEL: ALevelBoard = {
  id: 'ocr', name: 'OCR A Level (H446)',
  topics: [

  { id:'ocr-1', number:'1.1', name:'The Characteristics of Contemporary Processors', specPoints: [

    { id:'1.1.1', title:'CPU Architecture & the FDE Cycle',
      sections: [
        t('Von Neumann vs Harvard Architecture','<strong>Von Neumann</strong>: instructions and data share the same memory and bus. Simple, general-purpose, but the shared bus creates the "Von Neumann bottleneck". Used in most general-purpose computers. <strong>Harvard</strong>: physically separate memory and buses for instructions and data. The CPU can fetch the next instruction while accessing data simultaneously. Used in microcontrollers and signal processors (DSPs).'),
        tbl(['Feature','Von Neumann','Harvard'],
          [['Memory','Single shared memory','Separate instruction and data memory'],
           ['Bus','Single bus for instructions + data','Separate buses'],
           ['Speed','Bottleneck at shared bus','Faster — simultaneous fetch'],
           ['Complexity','Simpler','More complex hardware'],
           ['Use','General-purpose PCs','Embedded systems, DSPs, microcontrollers']]),
        t('The FDE Cycle','Fetch: PC → MAR; memory[MAR] → MDR → CIR; PC incremented. Decode: CU reads CIR. Execute: ALU operates or data moved or PC updated for branch. Interrupt check occurs at end of each cycle.'),
        kt('Interrupt','A signal to the CPU requesting attention — from hardware (I/O complete, hardware error) or software (exception, system call). After each FDE cycle, the CPU checks an interrupt register. If an interrupt is pending: save current state to stack, jump to Interrupt Service Routine (ISR), restore state afterwards.'),
      ]
    },

    { id:'1.1.2', title:'Pipelining & Parallel Processing',
      sections: [
        t('Pipelining','Overlaps FDE stages of consecutive instructions. While instruction N is executing, instruction N+1 is decoding, N+2 is fetching. Increases throughput without increasing clock speed. A 3-stage pipeline can theoretically process one instruction per clock cycle (after filling).'),
        tbl(['Hazard type','Cause','Solution'],
          [['Data hazard','Instruction needs result not yet computed','Pipeline stall (bubble); forwarding (bypass)'],
           ['Control hazard','Branch instruction — next instruction unknown until branch resolved','Branch prediction; speculative execution; pipeline flush on misprediction'],
           ['Structural hazard','Two instructions need same hardware resource simultaneously','Stall; duplicate hardware (e.g. separate instruction/data cache = Harvard)']]),
        kt('SIMD','Single Instruction Multiple Data — one instruction applied to multiple data elements simultaneously. The core of GPU architecture. Example: adding two arrays of 1000 numbers simultaneously rather than 1000 separate add instructions. Essential for graphics, machine learning.'),
        kt('Multicore processing','Multiple complete processor cores on one chip. Each core has its own registers, ALU, control unit and cache (usually shared L3). True parallel execution — different cores can run different programs (or different threads of one program) simultaneously.'),
        tip('"Compare pipelining with multicore processing." Answer: "Pipelining overlaps stages of sequential instruction processing within one core — it increases throughput for a single instruction stream. Multicore processing provides multiple independent cores for genuinely parallel execution — different programs or threads run simultaneously. Pipelining benefits single-threaded programs; multicore benefits multi-threaded/parallel programs."'),
      ]
    },
  ]},

  { id:'ocr-4', number:'1.4', name:'Data Types, Data Structures & Algorithms', specPoints: [

    { id:'1.4.1', title:'Abstract Data Types',
      sections: [
        kt('Abstract Data Type (ADT)','A data type defined by its behaviour (operations and their semantics) rather than its implementation. The user knows what operations are available and what they do, but not how they are implemented internally. Examples: stack, queue, linked list, binary tree, graph, dictionary.'),
        tbl(['ADT','Core operations','Typical implementation'],
          [['Stack','push, pop, peek, isEmpty, size','Array or linked list'],
           ['Queue','enqueue, dequeue, front, isEmpty, size','Circular array or linked list'],
           ['Priority queue','insert, remove-min/max, peek','Binary heap'],
           ['Dictionary / Map','get, put, delete, contains','Hash table or BST'],
           ['Set','add, remove, contains, union, intersection','Hash set or BST'],
           ['Graph','addVertex, addEdge, adjacent, neighbours','Adjacency list or matrix']]),
        kt('Adjacency list','Graph representation: a list (or dictionary) where each vertex has a list of its neighbours. Space-efficient for sparse graphs: O(V+E). Traversal efficient but edge lookup O(degree).'),
        kt('Adjacency matrix','Graph representation: a V×V matrix where matrix[i][j] = 1 (or weight) if edge exists. O(V²) space — wasteful for sparse graphs. Edge lookup O(1). Good for dense graphs.'),
      ]
    },

    { id:'1.4.2', title:'Tree Structures & Traversals',
      sections: [
        kt('Binary tree','A tree where each node has at most two children (left and right). If every node is either a leaf or has exactly two children, it is a <strong>full binary tree</strong>.'),
        kt('Binary search tree (BST)','A binary tree where every node in the left subtree has a smaller key than the node, and every node in the right subtree has a larger key. Enables O(log n) search, insert and delete on average. Degrades to O(n) if unbalanced.'),
        w('BST Traversals','Tree:     4\n        /   \\\n       2     6\n      / \\   / \\\n     1   3 5   7\n\nIn-order (Left, Root, Right):\n  1, 2, 3, 4, 5, 6, 7  ← sorted order! (BST property)\n\nPre-order (Root, Left, Right):\n  4, 2, 1, 3, 6, 5, 7  ← copies/serialises the tree\n\nPost-order (Left, Right, Root):\n  1, 3, 2, 5, 7, 6, 4  ← deletes tree / evaluates expressions'),
        tbl(['Traversal','Order','Use case'],
          [['In-order','Left → Root → Right','Get BST values in sorted order'],
           ['Pre-order','Root → Left → Right','Copy a tree; serialise; prefix expressions'],
           ['Post-order','Left → Right → Root','Delete a tree; evaluate expression trees; postfix']]),
        w('BST Insert and Search','class BSTNode:\n    def __init__(self, key):\n        self.key = key\n        self.left = self.right = None\n\ndef insert(root, key):\n    if root is None:\n        return BSTNode(key)\n    if key < root.key:\n        root.left  = insert(root.left, key)\n    elif key > root.key:\n        root.right = insert(root.right, key)\n    return root   # key == root.key: already present\n\ndef search(root, key):\n    if root is None or root.key == key:\n        return root\n    if key < root.key:\n        return search(root.left, key)\n    return search(root.right, key)'),
      ]
    },
  ]},
  ...
  { id:'ocr-2', number:'1.2', name:'Software & Software Development', specPoints: [
    ...
    { id:'1.2.1', title:'Development Methodologies',
      sections: [
        tbl(['Methodology','Approach','Advantages','Disadvantages'],
          [['Waterfall','Sequential: requirements → design → code → test → maintain','Simple; well-documented; works for stable requirements','Inflexible; errors found late; customer sees product only at end'],
           ['Agile (Scrum/XP)','Iterative sprints; continuous feedback; working software every 2–4 weeks','Flexible; customer involved throughout; changes accommodated','Less documentation; scope creep; harder to estimate cost/time'],
           ['Spiral','Iterative with risk analysis at each cycle','Risk-managed; suits large complex projects','Expensive; complex to manage'],
           ['RAD (Rapid Application Dev)','Prototyping + user feedback; reuse components','Fast delivery; user-centred','Requires experienced team; not for large systems']]),
        kt('Agile manifesto','Values individuals and interactions over processes and tools; working software over comprehensive documentation; customer collaboration over contract negotiation; responding to change over following a plan.'),
        tip('"When would Waterfall be preferred over Agile?" Answer: "Waterfall suits projects where requirements are well-defined and unlikely to change, such as safety-critical systems (medical devices, aviation software) where thorough documentation and formal approval at each stage are required. Agile is better when requirements will evolve and customer feedback should shape the product throughout development."'),
      ]
    },

    { id:'1.2.2', title:'Testing Strategies',
      sections: [
        kt('Unit testing','Testing individual functions/modules in isolation. Verifies that each unit works correctly independently. Uses mock objects to simulate dependencies. Fast to run; easy to automate.'),
        kt('Integration testing','Testing that multiple units work correctly together. Verifies interfaces between modules. Detects errors that unit tests cannot — incorrect data passed between modules.'),
        kt('System testing','Testing the complete system end-to-end against the system requirements. Checks the whole system behaves as specified.'),
        kt('Acceptance testing','Testing by the end user (or client) to verify the system meets their requirements. Final gate before deployment. Includes alpha testing (in-house) and beta testing (limited release to real users).'),
        kt('Regression testing','Re-running previously passing tests after a change to ensure the change has not broken anything. Essential in continuous integration. Automated test suites make this feasible.'),
        tbl(['Test type','What it tests','Who runs it'],
          [['Unit','Individual functions','Developer'],
           ['Integration','Module interactions','Developer/QA'],
           ['System','Complete system vs requirements','QA team'],
           ['Acceptance','System vs user needs','End user / client'],
           ['Regression','No new bugs from changes','Developer/QA (automated)'],
           ['White-box','Internal code paths','Developer'],
           ['Black-box','Inputs/outputs only','QA, user']]),
      ]
    },
  ]},
  ...
  { id:'ocr-3', number:'1.3', name:'Exchanging Data', specPoints: [
    ...
    { id:'1.3.1', title:'Protocols & Data Exchange',
      sections: [
        tbl(['Protocol','Layer','Purpose'],
          [['HTTP/HTTPS','Application','Web page transfer; HTTPS adds TLS encryption'],
           ['FTP','Application','File transfer between systems'],
           ['SMTP','Application','Sending email'],
           ['IMAP/POP3','Application','Receiving email (IMAP: server-based; POP3: downloads)'],
           ['DNS','Application','Domain name → IP address resolution'],
           ['TCP','Transport','Reliable, ordered, connection-oriented delivery'],
           ['UDP','Transport','Fast, connectionless, unreliable delivery'],
           ['IP','Internet','Packet addressing and routing'],
           ['ARP','Link','IP address → MAC address resolution on LAN']]),
        kt('Packet switching','Data divided into packets; each packet routed independently across the network; reassembled at destination using sequence numbers. Advantages: efficient use of network links (shared); resilient (packets reroute around failures). Contrasted with circuit switching (dedicated path reserved for duration of call — used in traditional telephony).'),
        kt('TCP/IP socket','An endpoint for communication defined by an IP address and port number. A web server listens on port 80 (HTTP) or 443 (HTTPS). When a client connects, both ends have a socket — the combination (client IP, client port, server IP, server port) uniquely identifies the connection.'),
      ]
    },

    { id:'1.3.2', title:'Network Security & Encryption',
      examTip:'Symmetric vs asymmetric key encryption is a classic OCR topic — know both and the role of each in TLS/HTTPS.',
      sections: [
        tbl(['Threat','Description','Mitigation'],
          [['Man-in-the-middle','Attacker intercepts and possibly alters traffic between two parties','TLS/HTTPS (mutual authentication + encryption)'],
           ['Packet sniffing','Capturing unencrypted network traffic','Encryption; VPN; avoid public WiFi for sensitive data'],
           ['SQL injection','Malicious SQL inserted via input fields to manipulate the database','Parameterised queries; input validation/sanitisation'],
           ['Cross-site scripting (XSS)','Malicious scripts injected into web pages viewed by others','Escape HTML output; Content Security Policy (CSP)'],
           ['Denial of Service (DoS)','Flood a server with requests to make it unavailable','Rate limiting; firewalls; CDN; bot detection'],
           ['Phishing','Fraudulent emails/sites tricking users into revealing credentials','User education; MFA; email filtering'],
           ['Brute force','Try all possible passwords systematically','Account lockout; CAPTCHA; strong password requirements']]),
        kt('Symmetric encryption','Both parties share the same secret key for encryption and decryption. Fast — suitable for bulk data. Problem: securely exchanging the key over an untrusted network. Examples: AES (Advanced Encryption Standard), DES.'),
        kt('Asymmetric encryption (public-key cryptography)','Each party has a public key (shared openly) and a private key (kept secret). Data encrypted with the public key can only be decrypted with the private key. Used for: key exchange, digital signatures. Slower than symmetric — used to establish a shared symmetric key, not for bulk data. Examples: RSA, elliptic curve cryptography (ECC).'),
        kt('TLS handshake (HTTPS)','1. Client sends "hello" with supported cipher suites. 2. Server sends certificate (containing public key). 3. Client verifies certificate against Certificate Authority (CA). 4. Client and server use asymmetric encryption to exchange a symmetric session key. 5. Further communication encrypted with the fast symmetric session key.'),
        kt('Digital certificate','Issued by a Certificate Authority (CA). Contains the server\'s public key, domain name, and CA\'s digital signature. The browser checks the CA\'s signature to verify the certificate is genuine — prevents impersonation. The CA is trusted by the OS/browser.'),
        kt('Hashing in security','A hash function maps any data to a fixed-size digest. Properties for security: deterministic, fast to compute, one-way (practically impossible to reverse), collision resistant (different inputs → different hashes). Used for: password storage (store hash, not password); file integrity checking; digital signatures (sign the hash rather than the full document).'),
        tip('"Why is asymmetric encryption used to set up HTTPS but symmetric encryption used for the actual data transfer?" Answer: "Asymmetric encryption is used during the handshake to securely exchange a symmetric session key — its mathematical properties allow this without a pre-shared secret. However, asymmetric encryption is 100–1000× slower than symmetric. Once both sides have the shared session key, symmetric encryption (AES) handles all actual data — giving both security and speed."'),
      ]
    },

    { id:'1.3.3', title:'Compression — Lossless & Lossy',
      sections: [
        tbl(['Type','Description','Quality preserved?','Example algorithms/formats'],
          [['Lossless','Original data can be reconstructed exactly','100%','Run-length encoding, Huffman coding, LZW, ZIP, PNG, FLAC'],
           ['Lossy','Some data permanently discarded; smaller file at cost of quality','Reduced','JPEG, MP3, AAC, H.264 (video)']]),
        kt('Run-length encoding (RLE)','Lossless. Replaces consecutive repeated values with (count, value) pairs. Highly effective for images with large areas of the same colour (icons, flags). Ineffective for photographic images with few repeating values.'),
        w('Run-length Encoding Example','Original pixel row (24 colours, simplified to 8):\nRRRRRRGGGGBBBBBBBBBR\n\nRLE encoded:\n(6,R)(4,G)(9,B)(1,R)  = 4 pairs × 2 values = 8 values\nvs 20 original values → 60% compression\n\nWorst case (no repeats):\nRGBRGBRGB → (1,R)(1,G)(1,B)(1,R)(1,G)(1,B)(1,R)(1,G)(1,B)\n→ 18 values vs 9 original → BIGGER! (no compression gain)'),
        kt('Huffman coding','Lossless variable-length encoding. Assigns shorter bit patterns to more frequent characters and longer patterns to rare ones. Builds a binary tree based on character frequencies. Used in ZIP, DEFLATE, and as a component in JPEG and MP3.'),
        w('Huffman Coding Process','Text: "AABBBCCCCDDDDDD"\nFrequencies: A=2, B=3, C=4, D=6\n\nBuild tree (lowest frequency first):\n1. {A:2,B:3} → merge → node(5)\n2. {C:4} and {node(5)} → merge → node(9)\n3. {D:6} and {node(9)} → merge → ROOT(15)\n\nTree:         ROOT(15)\n             /        \\\n           D(6)     node(9)\n                    /      \\\n                 C(4)     node(5)\n                          /    \\\n                        B(3)  A(2)\n\nCodes: D=0, C=10, B=110, A=111\n\nOriginal: AABBBCCCCDDDDDD\n  = 15 chars × 8 bits = 120 bits\n\nEncoded: 111 111 110 110 110 10 10 10 10 0 0 0 0 0 0\n  = 3+3+3+3+3+2+2+2+2+1+1+1+1+1+1 = 29 bits\nCompression ratio: 120/29 ≈ 4.1:1'),
      ]
    },
  ]},

  { id:'ocr-5', number:'1.5', name:'Legal, Moral, Cultural & Ethical Issues', specPoints: [
    { id:'1.5.1', title:'Ethical Frameworks in Computing',
      sections: [
        tbl(['Ethical framework','Approach','Applied to computing'],
          [['Consequentialism','Judge actions by their outcomes (greatest good for greatest number)','Is the harm caused by collecting data worth the benefits to users?'],
           ['Deontology','Some actions are inherently right/wrong regardless of consequences','Do not violate privacy even if the data could save lives'],
           ['Virtue ethics','What would a person of good character do?','Would a professional with integrity build this surveillance system?'],
           ['Social contract','Obligations arise from implicit agreements in society','Users expect data to be protected — breaching this violates the social contract']]),
        t('AI Ethics','Key concerns with artificial intelligence: <strong>Bias</strong> — AI trained on biased data produces biased decisions (facial recognition accuracy varies by race; hiring algorithms discriminate). <strong>Accountability</strong> — who is responsible when autonomous systems make harmful decisions? <strong>Transparency</strong> — "black box" AI decisions are difficult to explain or challenge. <strong>Autonomous weapons</strong> — should machines make lethal decisions?'),
        t('The Digital Divide','The gap between those with and without access to digital technology. Has multiple dimensions: geographic (rural vs urban), economic (rich vs poor), generational (young vs old), and international (developed vs developing nations). As essential services move online, the divide has serious social consequences for education, employment and healthcare access.'),
      ]
    },
  ]},

  { id:'ocr-6', number:'2.1', name:'Elements of Computational Thinking', specPoints: [
    { id:'2.1.1', title:'Computational Thinking Techniques',
      sections: [
        t('Decomposition','Breaking complex problems into smaller sub-problems. Each sub-problem can be solved independently and solutions combined. Enables parallel development, modular code, and easier testing. The key skill is identifying the right decomposition — what are the natural boundaries?'),
        t('Abstraction','Removing irrelevant detail, retaining only what matters for the solution. Two forms: <strong>data abstraction</strong> (hiding implementation details — a stack\'s "push" hides whether it uses an array or linked list internally) and <strong>problem abstraction</strong> (simplifying the problem model — a simulation of traffic flow abstracts away car makes, weather, individual driver behaviour).'),
        t('Algorithmic Thinking','Developing a step-by-step solution that can be expressed as an algorithm. Requires: identifying inputs and outputs, breaking the problem into steps, defining sequences/conditions/loops, and verifying correctness with test cases.'),
        kt('Pattern recognition','Identifying regularities, similarities or common structures within a problem or between problems. Enables reuse of existing solutions. Example: recognising that a new scheduling problem is similar to a shortest-path problem allows applying Dijkstra\'s algorithm.'),
        t('Backtracking','A technique for solving constraint-satisfaction problems. Build a solution step by step; when a step fails (violates a constraint), undo it and try another option. Used in: N-queens problem, Sudoku solvers, maze solving, parsing.'),
      ]
    },
  ]},

  { id:'ocr-7', number:'2.2', name:'Problem Solving & Programming', specPoints: [
    { id:'2.2.1', title:'OOP & Polymorphism (OCR)',
      sections: [
        t('Composition vs Inheritance','<strong>Inheritance</strong> (IS-A relationship): Dog IS-A Animal. The subclass gets all parent attributes/methods. Tight coupling — changes to parent affect all children. <strong>Composition</strong> (HAS-A relationship): Car HAS-A Engine. The class contains an instance of another class. More flexible — components can be swapped without rewriting the containing class. "Favour composition over inheritance" is a key OOP design principle.'),
        w('Composition Example','# Inheritance approach\nclass Duck(Bird):\n    def fly(self): ...\n    def quack(self): ...\n\n# Problems: RubberDuck inherits fly() but can\'t fly!\n\n# Composition approach — behaviours as separate classes\nclass CanFly:\n    def fly(self): return "Flying!"\n\nclass CannotFly:\n    def fly(self): return "Cannot fly"\n\nclass Duck:\n    def __init__(self):\n        self.fly_behaviour = CanFly()   # Has-a\n\nclass RubberDuck:\n    def __init__(self):\n        self.fly_behaviour = CannotFly()  # Different behaviour\n\n# Now adding new fly behaviours does not affect existing classes'),
        kt('Interface','A contract specifying what methods a class must implement, without defining how. In Python, achieved through abstract base classes (ABC). In Java/C#, interfaces are a language feature. Classes implementing an interface guarantee certain behaviours — essential for polymorphism.'),
      ]
    },
    { id:'2.2.2', title:'Recursion in Programming',
      sections: [
        w('Tower of Hanoi','Move n discs from peg A to peg C using peg B.\nRule: never place a larger disc on a smaller one.\n\ndef hanoi(n, source, target, auxiliary):\n    if n == 1:\n        print(f"Move disc 1 from {source} to {target}")\n        return\n    # Move n-1 discs from source to auxiliary\n    hanoi(n-1, source, auxiliary, target)\n    # Move disc n from source to target\n    print(f"Move disc {n} from {source} to {target}")\n    # Move n-1 discs from auxiliary to target\n    hanoi(n-1, auxiliary, target, source)\n\nhanoi(3, "A", "C", "B")\n# Requires 2ⁿ - 1 = 7 moves for n=3\n# Demonstrates: some problems are naturally recursive'),
        t('Memoisation','Caching the results of recursive function calls to avoid recomputation. Transforms exponential recursion into linear time for problems with overlapping subproblems. A key technique in dynamic programming.'),
        w('Memoisation Example (Fibonacci)','# Naive: O(2ⁿ) — recalculates same values many times\ndef fib(n):\n    if n <= 1: return n\n    return fib(n-1) + fib(n-2)\n\n# Memoised: O(n) — each value calculated once\ndef fib_memo(n, cache={}):\n    if n in cache: return cache[n]\n    if n <= 1: return n\n    cache[n] = fib_memo(n-1, cache) + fib_memo(n-2, cache)\n    return cache[n]\n\n# Or using Python decorator:\nfrom functools import lru_cache\n@lru_cache(maxsize=None)\ndef fib(n):\n    if n <= 1: return n\n    return fib(n-1) + fib(n-2)'),
      ]
    },
  ]},

  { id:'ocr-8', number:'2.3', name:'Algorithms', specPoints: [
    { id:'2.3.1', title:"A* Search Algorithm",
      examTip:"A* is unique to OCR H446 — understand how the heuristic improves on Dijkstra's.",
      sections: [
        t("A* Search","An extension of Dijkstra's algorithm that uses a <strong>heuristic function</strong> h(n) to estimate the cost from node n to the goal. The priority is f(n) = g(n) + h(n), where g(n) is the actual cost from the start to n. By prioritising nodes that are both close to the start (g) and estimated to be close to the goal (h), A* typically finds the shortest path exploring fewer nodes than Dijkstra's."),
        kt('Admissible heuristic','A heuristic h(n) that never overestimates the true cost to the goal. If h is admissible, A* is guaranteed to find the optimal (shortest) path. Example: straight-line (Euclidean) distance between two geographic locations is admissible for road navigation because the actual road distance cannot be shorter than the straight line.'),
        w("A* vs Dijkstra's","Dijkstra's: explores nodes in order of g(n) — actual distance from start\nA*: explores nodes in order of f(n) = g(n) + h(n)\n\nExample: finding path from A to Z in a map\n  Dijkstra's explores all directions equally (like water flooding)\n  A* focuses exploration toward Z using the heuristic\n  \n  Result: A* finds the same optimal path but explores far fewer nodes\n  \n  Trade-off: A* is faster but requires a good heuristic\n             Dijkstra's works on any weighted graph\n             A* needs domain knowledge (to design h)"),
        tbl(['Feature',"Dijkstra's",'A*'],
          [['Priority','g(n) — cost from start','f(n) = g(n) + h(n)'],
           ['Heuristic','None','Required (admissible for optimality)'],
           ['Optimal?','Yes (non-negative weights)','Yes (if h is admissible)'],
           ['Speed','Slower — explores all directions','Faster — guided by heuristic'],
           ['Use case','Any weighted graph','When good heuristic available (maps, games)']]),
      ]
    },
    { id:'2.3.2', title:'Complexity & NP Problems',
      sections: [
        t('P and NP Classes','<strong>P</strong>: decision problems solvable in polynomial time O(nᵏ). <strong>NP</strong>: decision problems where a YES answer can be <em>verified</em> in polynomial time, even if finding the answer might be harder. P ⊆ NP — all P problems are also in NP (a polynomial solution is also a polynomial verification). The biggest unsolved problem in CS: is P = NP?'),
        kt('NP-Complete','Problems that are in NP AND are at least as hard as every other problem in NP. If any NP-Complete problem can be solved in polynomial time, then P = NP (and every NP problem can be solved in polynomial time). Examples: Boolean Satisfiability (SAT), Travelling Salesman Problem (decision form), Graph Colouring.'),
        t('Implications for Practice','For NP-Complete problems in practice: (1) Use exact algorithms for small inputs where exponential is tolerable. (2) Use approximation algorithms — find near-optimal solutions in polynomial time. (3) Use heuristics — find good (not optimal) solutions quickly. (4) Exploit special structure — some NP-Complete problems have polynomial solutions on restricted inputs.'),
      ]
    },
  ]},
